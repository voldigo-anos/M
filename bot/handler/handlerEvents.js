...
// 🎯 UPDATED getRole function for roles 0-5
function getRole(threadData, senderID) {
		const config = global.GoatBot.config || {};
		const adminBot = config.adminBot || [];
		const developerUsers = config.developerUsers || []; 
	...
	if (!senderID)
		...
		// Role Hierarchy Check (Highest to Lowest)
		// developerUser role 5
		if (developerUsers.includes(senderID)) return 5;

		// vipUser role 4
		if (vipUsers.includes(senderID)) return 4;
...
function getText(type, reason, time, targetID, lang) {
	...
	if (type == "userBanned")
		...
		// Keys for new roles (assuming these keys exist in your language files)
		else if (type == "onlyAdminBot3")
		return utils.getText({ lang, head: "handlerEvents" }, "onlyAdminBot3");
		else if (type == "onlyVIP")
		return utils.getText({ lang, head: "handlerEvents" }, "onlyVIP");
		else if (type == "onlyDeveloper")
		return utils.getText({ lang, head: "handlerEvents" }, "onlyDeveloper");
		// 🎯 New Key for prefix-only message
		else if (type == "prefixOnly") {
				const userName = global.db.allUserData.find(u => u.userID == targetID)?.name || "User";
				return utils.getText({ lang, head: "handlerEvents" }, "prefixOnly", userName);
		}
		// ✅ NEW Key for pending approval message
		else if (type == "pendingApproval")
				return utils.getText({ lang, head: "handlerEvents" }, "pendingApproval", targetID); // targetID is commandName here
}
	...
	return async function (event, message) {
		...
		async function onStart() {
			...
			if (!command)
				if (!hideNotiMessage.commandNotFound)
					...
					return true;
						...
						// 🎯 NEW APPROVAL LOGIC CHECK (Whitelisting & Local Toggle)
						const isBotAdmin = role >= 3; // Role 3, 4, 5 are Bot Admins
						const isGroupAdmin = role >= 1; // Role 1 is Group Admin
						const isApprovalOn = threadData.data.commandApproval === true;

						// Check global whitelist (Bot Admin managed)
						// NOTE: Using 'whiteListThreadIds' here. Ensure your globalData key matches this!
						const globalDataDoc = await globalData.get("data", "data") || {};
						const whiteListThreadIds = globalDataDoc.whiteListThreadIds || []; 
						const isGloballyApproved = whiteListThreadIds.includes(threadID);

						// If approval is ON in this thread, AND it's NOT globally approved, AND the user is NOT a Group/Bot Admin, 
						// AND the command is NOT 'approval' (to prevent deadlock).
						if (isApprovalOn && !isGloballyApproved && !isGroupAdmin && !isBotAdmin && commandName !== 'approval') {
								if (!hideNotiMessage.pendingApproval)
										await message.reply(getText("pendingApproval", null, null, commandName, langCode));
			...
			if (needRole > role) {
				if (!hideNotiMessage.needRoleToUseCmd) {
										// Updated permission messages for new roles
					if (needRole == 1)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdmin", commandName)); // Group Admin
					else if (needRole == 2)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyPremium", commandName)); // Premium User
										else if (needRole == 3)
						return await message.reply(getText("onlyAdminBot3", null, null, null, langCode)); // Bot Admin
										else if (needRole == 4)
						return await message.reply(getText("onlyVIP", null, null, null, langCode)); // VIP User
										else if (needRole == 5)
						return await message.reply(getText("onlyDeveloper", null, null, null, langCode)); // Developer User
					else if (needRole >= 2) 
		...
		async function onReply() {
			...
			if (needRole > role) {
				if (!hideNotiMessage.needRoleToUseCmdOnReply) {
										// Updated permission messages for new roles
					if (needRole == 1)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminToUseOnReply", commandName)); // Group Admin
										else if (needRole == 2)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyPremiumToUseOnReply", commandName)); // Premium User
										else if (needRole == 3)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminBot3ToUseOnReply", commandName)); // Bot Admin
										else if (needRole == 4)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyVIPToUseOnReply", commandName)); // VIP User
										else if (needRole == 5)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyDeveloperToUseOnReply", commandName)); // Developer User
					else if (needRole >= 2)
		...
		*/
		async function onReaction() {
						...
						// 🎯 NEW: React to Delete Logic (for Bot Admins)
						const isBotAdmin = role >= 3; // Bot Admin roles 3, 4, 5
						const botID = api.getCurrentUserID();

						// Check if the reaction is 😠 and the message belongs to the bot
						if (event.reaction === '😠' && event.senderID !== botID) { // senderID is the user reacting, must not be the bot itself
								if (isBotAdmin) {
										// event.messageID is the ID of the message that received the reaction
										return api.unsendMessage(event.messageID)
												.catch(err => {
														log.err("ReactToDelete", `Failed to unsend message ${event.messageID}`, err);
												});
								}
						}
						// End of React to Delete Logic
			...
			if (needRole > role) {
				if (!hideNotiMessage.needRoleToUseCmdOnReaction) {
										// Updated permission messages for new roles
					if (needRole == 1)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminToUseOnReaction", commandName)); // Group Admin
										else if (needRole == 2)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyPremiumToUseOnReaction", commandName)); // Premium User
										else if (needRole == 3)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyAdminBot3ToUseOnReaction", commandName)); // Bot Admin
										else if (needRole == 4)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyVIPToUseOnReaction", commandName)); // VIP User
										else if (needRole == 5)
						return await message.reply(utils.getText({ lang: langCode, head: "handlerEvents" }, "onlyDeveloperToUseOnReaction", commandName)); // Developer User
					else if (needRole >= 2)
